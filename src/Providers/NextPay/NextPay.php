...
class NextPay extends AbstractProvider {
    ...
    protected $api_type = ApiType::SoapClient;
            ...
            case ApiType::Http:
                {
                    ...
                    curl_setopt($curl, CURLOPT_POSTFIELDS,
                        "api_key=" . $this->config['api'] . "&order_id=" . $transaction->getId() . "&amount=" . $transaction->getAmount()->getToman() . "&callback_uri=" . $this->getCallback($transaction));
            ...
            case ApiType::Http:
                {
                    ...
                    curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
                    curl_setopt($curl, CURLOPT_POSTFIELDS,
                        "api_key=" . $this->config['api'] . "&order_id=" . $order_id . "&amount=" . $transaction->getAmount()->getToman() . "&trans_id=" . $trans_id);
                    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
                    /** @var int | string $server_output */
                    $response = json_decode(curl_exec($curl));
                    curl_close($curl);
